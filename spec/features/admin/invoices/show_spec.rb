...
RSpec.describe "Admin Invoice Show" do
    before :each do
      ...
      @ii3 = InvoiceItem.create!(item_id: "#{@item3.id}", invoice_id: "#{@inv1.id}", quantity: 300, unit_price: 2000, status: 1)
      @ii4 = InvoiceItem.create!(item_id: "#{@item4.id}", invoice_id: "#{@inv2.id}", quantity: 300)
    end
    ...
    it "displays total revenue that will be generated by invoice" do
      visit("/admin/invoices/#{@inv1.id}")
      expect(page).to have_content("Total Revenue: 11000.00")
    end
    ...
    it 'shows total revenue for invoice and discounted revenue for invoice' do
